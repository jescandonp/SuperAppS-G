...
public static class PortalEndpoints
{
    public static IEndpointRouteBuilder MapPortalEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        {var denied=await authorization.RequireAsync("SCHEDULING","APPROVE_EXCEPTION",ct);if(denied is not null&&userContext.User is not null)await repository.AuditScheduleExceptionDenialAsync(versionId,userContext.User.Id,userContext.User.Username,ct);if(denied is not null)return denied;if(request.EvaluationId<=0||request.RuleCode is null||!System.Text.RegularExpressions.Regex.IsMatch(request.RuleCode,"^I9-R0[1-7]$")||string.IsNullOrWhiteSpace(request.MotiveCode)||request.MotiveCode.Length>50||!System.Text.RegularExpressions.Regex.IsMatch(request.MotiveCode,"^[A-Z0-9_]+$")||string.IsNullOrWhiteSpace(request.Reason)||string.IsNullOrWhiteSpace(request.Responsible)||!DateOnly.TryParse(request.ResolutionDate,out var date))return Results.BadRequest(new{message="Evaluacion, regla, motivo, responsable y fecha son obligatorios."});try{return Results.Ok(await repository.CreateScheduleExceptionAsync(versionId,request,date,userContext.User!.Id,userContext.User.Username,ct));}catch(Exception ex)when(ex is InvalidOperationException or System.Data.DBConcurrencyException){return Results.Conflict(new{message=ex.Message});}catch(KeyNotFoundException){return Results.NotFound();}});
