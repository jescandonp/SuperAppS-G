...
export function SchedulingPage({ user }: Props) {
  ...
  const simulatedOrigin = demoMode || (ruleProfile.status === "READY" && ruleProfile.profile.simulated);
  ...
  async function compare() { if (!proposal) return; setBusy(true); try { const next = demoMode ? [{ mode: "MINIMUM_IMPACT", changedAssignments: 3, additionalHours: 4, vacancies: 1, exceptions: 1 }, { mode: "GLOBAL", changedAssignments: 8, additionalHours: 0, vacancies: 0, exceptions: 2 }] as ScheduleComparison[] : (await replanSchedule(proposal.versionId, { triggerType: "MANUAL_REVIEW", triggerId: String(proposal.versionId), modes: ["MINIMUM_IMPACT", "GLOBAL"] })).scenarios; setScenarios(next); setTab("Comparar"); } finally { setBusy(false); } }
  async function registerException(reason: string) { if (!proposal || demoMode) { setMessage(`Desviación demostrativa registrada: ${reason}`); return; } const next = await approveScheduleException(proposal.versionId, { exceptionType: "DESVIACION_PLANTILLA", reason, responsible: user.fullName, resolutionDate: periodEnd, expectedVersion: proposal.versionNumber }); setProposal(next); }
  ...
  async function loadRuleProfile(code: string) {
    setRuleProfile({ status: "LOADING" });
    try {
      const active = (await fetchSchedulingRuleProfiles(code, periodStart, "MVP_TEST")).filter((item) => item.status === "ACTIVE");
  ...
  async function exportFile(format: "pdf" | "xlsx") { if (!proposal) return; setBusy(true); try { if (!demoMode) await (format === "pdf" ? downloadSchedulePdf(proposal.versionId) : downloadScheduleXlsx(proposal.versionId)); else setMessage(`Exportación ${format.toUpperCase()} simulada en modo demostrativo.`); } finally { setBusy(false); } }
