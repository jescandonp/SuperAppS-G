...
# xref -> (nombre de archivo, extension real detectada por extract_image)
ASSETS = {
    11: "header",
    12: "footer",
    5: "watermark",
...
def main():
    ...
    for xref, name in ASSETS.items():
        info = doc.extract_image(xref)
        out_path = os.path.join(OUTPUT_DIR, f"{name}.{info['ext']}")
        with open(out_path, "wb") as f:
            f.write(info["image"])
        print(f"{name}: {out_path} ({len(info['image'])} bytes, {info['width']}x{info['height']})")
