param(
    ...
    [string]$VerificationSchema = "i2_verify_clean"
)
...
$scripts = @(
    ...
    (Join-Path $workspaceRoot "db\tests\002_i2_persistence_contract.sql")
)
...
try {
    ...
    foreach ($script in $scripts) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Clean I2 persistence verification failed while executing $script."
        }
    }
}
finally {
