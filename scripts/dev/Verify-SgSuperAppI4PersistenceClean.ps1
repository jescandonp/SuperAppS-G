param(
    ...
    [string]$VerificationSchema = "i4_verify_clean"
)
...
$scripts = @(
    ...
    (Join-Path $workspaceRoot "db\tests\004_i4_persistence_contract.sql")
)
...
try {
    ...
    foreach ($script in $scripts) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Clean I4 persistence verification failed while executing $script."
        }
    }
}
finally {
