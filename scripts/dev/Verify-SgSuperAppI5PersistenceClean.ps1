param(
    ...
    [string]$VerificationSchema = "i5_verify_clean"
)
...
$scripts = @(
    ...
    (Join-Path $workspaceRoot "db\tests\005_i5_persistence_contract.sql")
)
...
try {
    ...
    foreach ($script in $scripts) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Clean I5 persistence verification failed while executing $script."
        }
    }
}
finally {
