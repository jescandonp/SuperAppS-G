...
Assert-DocumentContains $specPath @(
    ...
    '(?m)^> Gate: 0 cerrado; Tasks 2, 3, 4 y 5 completadas; Task 6 pendiente\s*$',
    '(?m)^Estado de aplicacion: \*\*TASK_5_COMPLETADA_TASK_6_PENDIENTE\*\*\s*$',
    '(?i)alcance',
...
Assert-DocumentContains $planPath @(
    ...
    '(?m)^> Estado general: \*\*Gate 0 cerrado - Tasks 2, 3, 4 y 5 completadas - Task 6 pendiente\*\*\s*$',
    '(?m)^> Gate 0: \*\*Cerrado\*\*\s*$',
    '(?i)Task 4.*completada',
    '(?i)Task 6.*pendiente',
    '(?is)Tasks 2, 3, 4 y 5.*completadas',
    'docs/superpowers/plans/2026-07-29-sg-programacion-turnos-implementation-plan\.md',
    '(?i)APROBADO COMO HOJA DE RUTA DOCUMENTAL',
    '(?i)TASK 5 COMPLETADA',
    '(?i)TASK 6 PENDIENTE',
    ...
    '(?is)Task 5.*completada.*Task 6.*pendiente',
    '(?is)Camilo\s+Piedrahita.*Gerente\s+General.*cierre',
    '(?i)GH-DE-01',
    '24/07/2025',
    '(?i)version 4',
...
Assert-PatternCount $planPath '(?m)^> Estado general: \*\*Gate 0 cerrado - Tasks 2, 3, 4 y 5 completadas - Task 6 pendiente\*\*\s*$' 1
