...
Assert-DocumentContains $specPath @(
    ...
    '(?m)^> Gate: 0 cerrado; Tasks 2 a 12 completadas; Task 13 pendiente\s*$',
    '(?m)^Estado de aplicacion: \*\*TASK_12_COMPLETADA_TASK_13_PENDIENTE\*\*\s*$',
    '(?i)alcance',
...
Assert-DocumentContains $planPath @(
    ...
    '(?m)^> Estado general: \*\*Gate 0 cerrado - Tasks 2 a 12 completadas - Task 13 pendiente\*\*\s*$',
    '(?m)^> Gate 0: \*\*Cerrado\*\*\s*$',
    '(?i)Task 4.*completada',
    '(?i)Task 13.*pendiente',
    ...
    '(?is)Task 12.*completada.*Task 13.*pendiente',
    '(?is)Camilo\s+Piedrahita.*Gerente\s+General.*cierre',
    '(?i)GH-DE-01',
    '24/07/2025',
    '(?i)version 4',
...
Assert-PatternCount $planPath '(?m)^> Estado general: \*\*Gate 0 cerrado - Tasks 2 a 12 completadas - Task 13 pendiente\*\*\s*$' 1
