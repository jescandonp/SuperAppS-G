...
Assert-DocumentContains $specPath @(
    ...
    '(?m)^> Gate: 0 cerrado; Tasks 2, 3, 4, 5 y 6 completadas; Task 7 pendiente\s*$',
    '(?m)^Estado de aplicacion: \*\*TASK_6_COMPLETADA_TASK_7_PENDIENTE\*\*\s*$',
    '(?i)alcance',
...
Assert-DocumentContains $planPath @(
    ...
    '(?m)^> Estado general: \*\*Gate 0 cerrado - Tasks 2, 3, 4, 5 y 6 completadas - Task 7 pendiente\*\*\s*$',
    '(?m)^> Gate 0: \*\*Cerrado\*\*\s*$',
    '(?i)Task 4.*completada',
    '(?i)Task 7.*pendiente',
    '(?is)Tasks 2, 3, 4, 5 y 6.*completadas',
    'docs/superpowers/plans/2026-07-29-sg-programacion-turnos-implementation-plan\.md',
    '(?i)APROBADO COMO HOJA DE RUTA DOCUMENTAL',
    '(?i)TASK 6 COMPLETADA',
    '(?i)TASK 7 PENDIENTE',
    ...
    '(?is)Task 6.*completada.*Task 7.*pendiente',
    '(?is)Camilo\s+Piedrahita.*Gerente\s+General.*cierre',
    '(?i)GH-DE-01',
    '24/07/2025',
    '(?i)version 4',
...
Assert-PatternCount $planPath '(?m)^> Estado general: \*\*Gate 0 cerrado - Tasks 2, 3, 4, 5 y 6 completadas - Task 7 pendiente\*\*\s*$' 1
