...
function Test-NoveltyRequirementLinkage {
    ...
    return $EvaluatorContent -match '(?s)"I9-R04"\s*=>\s*EvaluateNoveltyRequirementRule\s*\(\s*"EvaluateR04"\s*,\s*entry\.Parameters\s*,\s*entry\.CatalogSnapshot\s*,\s*sanitizedFacts' -and
        ...
        $RulesContent -match 'mappingDemo' -and $RulesContent -match '(?s)activeMappings\.Length\s*>\s*1.*?SchedulingRuleContractException' -and
...
$implementations = @(
    ...
    @{ R='I9-R04/R06 rules'; P='apps/sg-superapp-api/Services/SchedulingNoveltyRequirementRules.cs'; X=@((Pattern 'R04 entry point' 'EvaluateR04\s*\('),(Pattern 'R06 entry point' 'EvaluateR06\s*\('),(Pattern 'unknown fail closed' '(?i)(UNKNOWN|UNVERIFIED)'),(Pattern 'anonymous employee key' '(?i)employeeId'),(Pattern 'position scope' '(?i)positionCode'),(Pattern 'canonical novelty mapping' '(?i)mappingDemo'),(Pattern 'versioned requirements catalog' '(?i)requirementsDemo'),(Pattern 'whole shift validity' '(?i)validForEntireShift'),(Pattern 'evidence traceability' '(?i)evidenceId'),(Pattern 'informative traceability' '(?is)remediationOwnerRole.*remediationOwnerKey.*dueDate')) },
    @{ R='I9-R06 persisted TH validation'; P='apps/sg-superapp-api/Services/SchedulingRuleEvaluator.cs'; X=@((Pattern 'persisted TH table' 'scheduling_rule_hr_validations'),(Pattern 'server validation write' 'ValidateRequirementEvidenceAsync'),(Pattern 'denial audit' 'SCHEDULE_REQUIREMENT_VALIDATION_DENIED')) },
    @{ R='I9-R06 TH authorization boundary'; P='apps/sg-superapp-api/Endpoints/SchedulingRuleEndpoints.cs'; X=@((Pattern 'technical permission' 'VALIDATE_REQUIREMENT'),(Pattern 'server actor' 'User!?\.Id'),(Pattern 'validation endpoint' 'hr-validations')) },
...
$focused = @(
    ...
    @{ R='I9-R04/R06 verifier'; P='scripts/dev/Verify-SgSuperAppI9R04R06.ps1'; Pass='I9 R04 R06 PASS 55' },
