...
foreach ($pattern in @(
    'scheduling_rule_hr_validations', 'VALIDATE_REQUIREMENT', 'ValidateRequirementEvidenceAsync',
    'SCHEDULE_REQUIREMENT_VALIDATION_DENIED', 'TH validation history is immutable',
    'uq_schedule_exceptions_evaluation_rule_scope_motive', 'schedule_exceptions_i9_immutable',
    "status IN\('ACTIVE','RETIRED'\)", 'contains a partial evaluation/rule/scope/motive identity',
    'contains duplicate evaluation/rule/scope/motive identities',
    'Ya existe una decision para la misma evaluacion'
)) {
...
// 2026-09-03: a disabled rule is NOT_APPLICABLE (the profile declares it out of scope), not WARNING -
// it never accredits compliance itself, but it also does not hold the gate closed on its own anymore.
var disabled4=E4(G4(N("FREE","ACTIVE","AVAILABLE")),enabled:false); Rule(R(disabled4,R04),SchedulingRuleOutcome.NOT_APPLICABLE,SchedulingRuleSeverity.INFO,"I9_R04_DISABLED_NOT_APPLICABLE",false,"R04-T24"); Summary(disabled4,1,0,0,0,0,1,true,"R04-T24"); Done("R04-T24");
...
var disabled6=E6(G6(allEvidence),allCatalog,enabled:false); Rule(R(disabled6,R06),SchedulingRuleOutcome.NOT_APPLICABLE,SchedulingRuleSeverity.INFO,"I9_R06_DISABLED_NOT_APPLICABLE",false,"R06-T23"); Summary(disabled6,1,0,0,0,0,1,true,"R06-T23"); Done("R06-T23");
...
'@ | Set-Content -LiteralPath (Join-Path $tempRoot 'Program.cs') -Encoding utf8
    ...
    # Build an isolated, source-current API assembly. The normal bin/obj may be
    # locked by a running local API and must never make the verifier use stale code.
    $apiSourceRoot = Join-Path $repoRoot 'apps/sg-superapp-api'
    $apiBuildRoot = Join-Path $tempRoot 'api-source'
    New-Item -ItemType Directory -Path $apiBuildRoot | Out-Null
    Get-ChildItem -LiteralPath $apiSourceRoot -Recurse -File |
        Where-Object { $_.FullName -notmatch '[\\/](bin|obj)[\\/]' } |
        ForEach-Object {
            $relative = $_.FullName.Substring($apiSourceRoot.Length).TrimStart('\')
            $destination = Join-Path $apiBuildRoot $relative
            New-Item -ItemType Directory -Path (Split-Path $destination) -Force | Out-Null
            Copy-Item -LiteralPath $_.FullName -Destination $destination
        }
    $isolatedApiProject = Join-Path $apiBuildRoot 'sg-superapp-api.csproj'
    & $dotnet restore $isolatedApiProject --ignore-failed-sources | Out-Null
    if ($LASTEXITCODE -ne 0) { Write-Output 'I9 R04 R06 BLOCKED: isolated API restore unavailable'; exit 2 }
    & $dotnet build $isolatedApiProject --configuration Release --no-restore | Out-Null
    if ($LASTEXITCODE -ne 0) { Write-Output 'I9 R04 R06 FAIL: source-current API build failed'; exit 1 }
...
var validator=new SchedulingRuleProfileValidator();var profileRepository=new SchedulingRuleProfileRepository(config,validator);var httpRepository=new SchedulingRuleHttpRepository(config);var evaluator=new SchedulingRuleEvaluator();var eligibilityService=new SchedulingEligibilityService(evaluator);var recommendationEngine=new SchedulingRecommendationEngine();var portalRepository=new PostgresPortalRepository(config,evaluator,profileRepository,httpRepository,eligibilityService,recommendationEngine);
...
// Task 22 - editing an assignment changes the facts the rules were decided on. The decision and
// the evaluation are append-only by trigger, so neither is rewritten: what changes is that the
// decision no longer stands, it stops counting in the summary, and the evaluation admits no new one.
long editEmployeeId,editReplacementId,editAssignmentId;
await using(var cn=new NpgsqlConnection(connectionString)){await cn.OpenAsync();
 await using var seed=new NpgsqlCommand(@"with e as (insert into employees(identification_type,identification_number,full_name,employment_status,job_title,hire_date) values('CC','I9-EDIT-1','I9 Edit Guard One','ACTIVO','GUARDA',date '2026-01-01'),('CC','I9-EDIT-2','I9 Edit Guard Two','ACTIVO','GUARDA',date '2026-01-01') returning id),
p as (insert into service_positions(code,name,status) values('I9-EDIT-POSITION','I9 Edit Position','ACTIVO') returning id),
r as (insert into required_shifts(schedule_version_id,position_id,shift_date,starts_at,ends_at) select @version,id,date '2026-08-21',time '08:00',time '20:00' from p returning id),
a as (insert into schedule_assignments(schedule_version_id,required_shift_id,employee_id,status) select @version,r.id,(select min(id) from e),'ASIGNADA' from r returning id)
select (select min(id) from e),(select max(id) from e),(select id from a)",cn);
 seed.Parameters.AddWithValue("version",versionId);
 await using var seedReader=await seed.ExecuteReaderAsync();await seedReader.ReadAsync();editEmployeeId=seedReader.GetInt64(0);editReplacementId=seedReader.GetInt64(1);editAssignmentId=seedReader.GetInt64(2);}
Q(editEmployeeId!=editReplacementId,"GEN-T11 edit fixture");
var editBatch=evaluator.Evaluate(profile,"PROJECT-A",new DateOnly(2026,8,21),cleanFacts);
var editSaved=await httpRepository.PersistEvaluationsAsync(versionId,editAssignmentId,"PROJECT-A",editBatch,"i9.demo.actor",CancellationToken.None);
var editR04=editSaved.Single(x=>x.Evaluation.RuleCode=="I9-R04");
await portalRepository.CreateScheduleExceptionAsync(versionId,new(editAssignmentId,editR04.Id,"I9-R04",editR04.Evaluation.ScopeHash,"HR_VALIDATED_DEMO","edit","OPS","2026-08-22",1),new DateOnly(2026,8,22),actorId,"i9.demo.actor");
var beforeEdit=await DecisionState(connectionString,editR04.Id);Q(beforeEdit.Decision=="APPROVED"&&!beforeEdit.Superseded,"GEN-T11 decision stands before the edit");
var summaryBeforeEdit=(await portalRepository.GetScheduleVersionAsync(versionId,CancellationToken.None))!.ExceptionCount;
var supersededAuditBefore=await SupersededAuditCount(connectionString,versionId);
await portalRepository.UpdateScheduleAssignmentAsync(versionId,editAssignmentId,new(editReplacementId,"ASIGNADA",new[]{"Cambio manual de guarda"},1),actorId,"i9.demo.actor");
var afterEdit=await DecisionState(connectionString,editR04.Id);
Q(afterEdit.Decision=="APPROVED"&&afterEdit.DecidedBy==beforeEdit.DecidedBy,"GEN-T11 the approval record survives the edit untouched");
Q(afterEdit.Superseded,"GEN-T12 the edit supersedes the decision taken on the previous facts");
Q(await SupersededAuditCount(connectionString,versionId)==supersededAuditBefore+1,"GEN-T12 the edit audits how many decisions it supersedes");
var summaryAfterEdit=(await portalRepository.GetScheduleVersionAsync(versionId,CancellationToken.None))!.ExceptionCount;
Q(summaryAfterEdit==summaryBeforeEdit-1,"GEN-T13 a superseded decision stops counting in the summary");
var staleExceptionsBefore=await ExceptionCount(connectionString,editR04.Id);var staleAuditBefore=await ApprovedAuditCount(connectionString,editR04.Id);var staleRejected=false;
try{await portalRepository.CreateScheduleExceptionAsync(versionId,new(editAssignmentId,editR04.Id,"I9-R04",editR04.Evaluation.ScopeHash,"HR_VALIDATED_DEMO","stale","OPS","2026-08-22",1),new DateOnly(2026,8,22),actorId,"i9.demo.actor");}
catch(InvalidOperationException ex) when(ex.Message.IndexOf("obsoleta",StringComparison.Ordinal)>=0){staleRejected=true;}
