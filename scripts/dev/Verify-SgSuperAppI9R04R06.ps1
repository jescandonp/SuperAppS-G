...
foreach ($pattern in @(
    ...
    'evidenceId', 'evidenceSource', 'sourceRequirementCode', 'hrValidation', 'validationId',
    'validatorRoleKey', 'validatedAt', 'remediationOwnerRole', 'remediationOwnerKey',
...
if ($evaluator -match '"hrValidated"') { Write-Output 'I9 R04 R06 FAIL: fabricated global hrValidated flag'; exit 1 }
...
var duplicateCat=Cat4(Map("INC","ACTIVE","INCAPACITY_ACTIVE","MAP-2")); var duplicateRejected=false; try { E4(G4(N("INC","ACTIVE","INCAPACITY_ACTIVE")),duplicateCat); } catch(SchedulingRuleContractException) { duplicateRejected=true; } Q(true,duplicateRejected,"R04-T23 rejected before decision"); Done("R04-T23");
...
string Ev(string code,string category,string state="VERIFIED",string version="REQ-1",string from=Start,string to=End,string extra="",string source="I3-DEMO",string sourceCode="",string evidenceId="",bool hr=false) { var evidence=evidenceId.Length==0?"EVID-"+code:evidenceId; var validation=hr?$",\"hrValidation\":{{\"validationId\":\"TH-{evidence}\",\"validatorRoleKey\":\"TH-VALIDATOR\",\"validatedAt\":\"2026-08-20T12:00:00-05:00\",\"evidenceId\":\"{evidence}\"}}":""; return $"{{\"evaluationId\":\"EVAL-{code}\",\"requirementCode\":\"{code}\",\"category\":\"{category}\",\"catalogVersion\":\"{version}\",\"sourceSystem\":\"{source}\",\"sourceRequirementCode\":\"{(sourceCode.Length==0?"SRC-"+code:sourceCode)}\",\"sourceStatus\":\"ACTIVE\",\"evidenceId\":\"{evidence}\",\"evidenceSource\":\"{source}\",\"evidenceType\":\"DEMO_RECORD\",\"evidenceState\":\"{state}\",\"validFrom\":\"{from}\",\"validTo\":\"{to}\"{validation}{extra}}}"; }
...
var hrMissing=Ev("COURSE-1","COURSE",state:"MISSING",hr:true); var hrPending=E6(G6(hrMissing),courseCat); Rule(R(hrPending,R06),SchedulingRuleOutcome.EXCEPTION_REQUIRED,SchedulingRuleSeverity.WARNING,"I9_R06_MISSING",true,"R06-T12"); Summary(hrPending,1,0,0,1,0,0,false,"R06-T12"); WorkflowPending("R06-T12");
var hrApproved=E6(G6(hrMissing),courseCat,new HashSet<string>{R(hrPending,R06).ScopeHash}); Summary(hrApproved,1,0,0,1,0,0,true,"R06-T13"); Q(R(hrPending,R06).ScopeHash,R(hrApproved,R06).ScopeHash,"R06-T13 exact hash"); WorkflowPending("R06-T13");
...
var deniedRepo6=new PostgresPortalRepository(false); var auth6=new PortalAuthorizationService(new RequestUserContext{User=new HarnessUser(6)},deniedRepo6); var denied6=await auth6.RequireAsync("SCHEDULING","APPROVE_EXCEPTION",CancellationToken.None); Q(403,await HttpStatus(denied6!),"R06-T15 status"); Q("SCHEDULING",deniedRepo6.LastModuleCode!,"R06-T15 module"); Q("APPROVE_EXCEPTION",deniedRepo6.LastActionCode!,"R06-T15 action"); WorkflowPending("R06-T15");
var oldHash=R(hrPending,R06).ScopeHash; var otherTurn=E6(G6(hrMissing,shift:"SHIFT-2"),courseCat,new HashSet<string>{oldHash}); var otherRequirementCat=Cat6(Req("COURSE-2","COURSE")); var otherRequirementEvidence=Ev("COURSE-2","COURSE",state:"MISSING",hr:true); var otherRequirement=E6(G6(otherRequirementEvidence),otherRequirementCat,new HashSet<string>{oldHash}); Summary(otherTurn,1,0,0,1,0,0,false,"R06-T16 turn"); Summary(otherRequirement,1,0,0,1,0,0,false,"R06-T16 requirement"); Q(false,oldHash==R(otherTurn,R06).ScopeHash,"R06-T16 turn hash"); Q(false,oldHash==R(otherRequirement,R06).ScopeHash,"R06-T16 requirement hash"); WorkflowPending("R06-T16");
